...
public class QuestionsAcademicControllerIntegrationTest {
    ...
    public void testGetExamQuestionById() throws Exception {
        int testId = 1;
        mockMvc.perform(get("/api/Academic/{id}", testId))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
    ...
    public void testDeleteExamQuestion() throws Exception {
        int testId = 1;
        mockMvc.perform(delete("/api/Academic/{id}", testId))
    ...
    public void testUpdateExamQuestion() throws Exception {
        int testId = 1;
        AcademicQuestionsDto preguntaDto = new AcademicQuestionsDto(testId, "Al resolver la ecuación: 2x+1=5, obtenemos", Grades.SEVENTH, "3", "2", "1", "5", "B", null);

        mockMvc.perform(put("/api/Academic/{id}", testId)
