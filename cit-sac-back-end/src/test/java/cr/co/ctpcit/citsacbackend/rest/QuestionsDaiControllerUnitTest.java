...
@WebMvcTest(QuestionsDaiController.class)
public class QuestionsDaiControllerUnitTest {
    ...
    void setUp() {
        ...
        question1 = new DaiQuestionsDto(1, "¿Puedes contarme sobre una vez en la que enfrentaste una situación difícil en la escuela y cómo la manejaste?", Grades.SEVENTH, null);
        question2 = new DaiQuestionsDto(5, "¿Cómo sueles manejar el estrés o la presión, especialmente durante los exámenes o proyectos importantes?", Grades.FIFTH, "imagen.jpg");
    }
    ...
    void getExamQuestionByIdNotFound() throws Exception {
        when(daiQuestionsService.obtenerPreguntaPorId(1)).thenThrow(new NoSuchElementException("Pregunta no encontrada con el id 1"));
    ...
    void deleteExamQuestion() throws Exception {
        mockMvc.perform(delete("/api/Dai/1")
                        .contentType(MediaType.APPLICATION_JSON))
    ...
    void deleteExamQuestionNotFound() throws Exception {
        doThrow(new NoSuchElementException("Pregunta no encontrada con el id 1")).when(daiQuestionsService).eliminarPregunta(1);
    ...
    void updateExamQuestion() throws Exception {
        DaiQuestionsDto updatedQuestion = new DaiQuestionsDto(1, "¿Cómo te has sentido las últimas semanas?", Grades.FORTH, null);
        when(daiQuestionsService.modificarPregunta(1, updatedQuestion)).thenReturn(updatedQuestion);

        mockMvc.perform(put("/api/Dai/1")
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    void updateExamQuestionNotFound() throws Exception {
        DaiQuestionsDto updatedQuestion = new DaiQuestionsDto(1, "¿Cómo te has sentido las últimas semanas?", Grades.FORTH, null);
        when(daiQuestionsService.modificarPregunta(1, updatedQuestion)).thenThrow(new NoSuchElementException("Pregunta no encontrada con el id 1"));
