...
public class QuestionsAcademicControllerUnitTest {
    ...
    void setUp() {
        ...
        question1 = new AcademicQuestionsDto(1, "¿Cuál es el río más largo del mundo?", Grades.SEVENTH, "Nilo", "Amazonas", "Yangtsé", "Misisipi", "B", null);
        question2 = new AcademicQuestionsDto(2, "¿Quién escribió La Odisea?", Grades.FIFTH, "Homero", "Virgilio", "Dante", "Shakespeare", "A", null);
    }
    ...
    void getExamQuestionById() throws Exception {
        ...
        mockMvc.perform(get("/api/Academic/1")
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
    ...
    void getExamQuestionByIdNotFound() throws Exception {
        when(academicQuestionsService.obtenerPreguntaPorId(1)).thenThrow(new NoSuchElementException("Pregunta no encontrada con el id 1"));
    ...
    void deleteExamQuestionNotFound() throws Exception {
        doThrow(new NoSuchElementException("Pregunta no encontrada con el id 1")).when(academicQuestionsService).eliminarPregunta(1);
    ...
    void updateExamQuestion() throws Exception {
        AcademicQuestionsDto updatedQuestion = new AcademicQuestionsDto(1, "¿Cuál es el océano más grande del mundo?", Grades.FORTH, "Atlántico", "Pacífico", "Índico", "Ártico", "B", null);
        when(academicQuestionsService.modificarPregunta(1, updatedQuestion)).thenReturn(updatedQuestion);

        mockMvc.perform(put("/api/Academic/1")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"id\":1,\"questionText\":\"¿Cuál es el océano más grande del mundo?\",\"questionGrade\":\"FORTH\",\"option_A\":\"Atlántico\",\"option_B\":\"Pacífico\",\"option_C\":\"Índico\",\"option_D\":\"Ártico\",\"correctOption\":\"B\",\"imageUrl\":null}"))
                .andExpect(status().isOk())
    ...
    void updateExamQuestionNotFound() throws Exception {
        AcademicQuestionsDto updatedQuestion = new AcademicQuestionsDto(1, "¿Cuál es el océano más grande del mundo?", Grades.FORTH, "Atlántico", "Pacífico", "Índico", "Ártico", "B", null);
        when(academicQuestionsService.modificarPregunta(1, updatedQuestion)).thenThrow(new NoSuchElementException("Pregunta no encontrada con el id 1"));
