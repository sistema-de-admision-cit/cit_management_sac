...
class InscriptionFormControllerTest {
  ...
  public void testCreateInscription_shouldCreateANewInscription() throws Exception {
    ...
    MockMultipartFile grades =
        new MockMultipartFile("grades", "grades.pdf", MediaType.APPLICATION_PDF_VALUE,
            "Some data".getBytes(StandardCharsets.UTF_8));

    MockMultipartFile inscriptionFile =
        new MockMultipartFile("inscription", "inscription", MediaType.APPLICATION_JSON_VALUE,
            inscription.getBytes(StandardCharsets.UTF_8));
    // Act & Assert
    mvc.perform(multipart("/api/inscription/add").file(grades).file(inscriptionFile))
