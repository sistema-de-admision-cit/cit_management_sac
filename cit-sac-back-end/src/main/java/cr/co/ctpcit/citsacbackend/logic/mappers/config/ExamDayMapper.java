...
public class ExamDayMapper {
  public static ExamDayDto convertToDto(ExamDayEntity examDay) {
    return new ExamDayDto(examDay.getExamDayId(), examDay.getExamPeriod().getExamPeriodId(),
        examDay.getExamDay(), examDay.getStartTime());
  }
  ...
  public static List<ExamDayDto> convertToDtoList(List<ExamDayEntity> examDays) {
    return examDays.stream().map(ExamDayMapper::convertToDto).toList();
  }
}
