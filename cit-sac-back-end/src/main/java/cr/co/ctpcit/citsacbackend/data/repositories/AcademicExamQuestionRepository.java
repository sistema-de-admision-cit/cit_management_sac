...
public interface AcademicExamQuestionRepository extends JpaRepository<AcademicExamQuestionsEntity, AcademicExamQuestionsEntityId> {
    @Query("SELECT CASE WHEN COUNT(a) > 0 THEN true ELSE false END FROM AcademicExamQuestionsEntity a WHERE a.id.questionId = :questionId")
